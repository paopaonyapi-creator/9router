...
// Kiro carries system/thinking instructions in the first user turn's content.
const firstUserContent = (body) => {
  const state = body.conversationState;
  return (state.history.find((turn) => turn.userInputMessage) || state.currentMessage)
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("injects thinking_mode tag when model implies thinking", () => {
    ...
    expect(firstUserContent(out)).toContain(
  ...
  it("does not send additionalModelRequestFields for Kiro models without effort support", () => {
    ...
    expect(firstUserContent(out)).toContain("<max_thinking_length>24576</max_thinking_length>");
  });
  ...
  it("normalizes an unsupported Kiro intensity suffix while preserving agentic behavior", () => {
    ...
    expect(out.additionalModelRequestFields).toBeUndefined();
    expect(firstUserContent(out)).toContain("CHUNKED WRITE PROTOCOL");
  });
  ...
  it("maps Claude-format effort to GPT-5.6 reasoning fields without legacy prompt tags", () => {
    ...
    expect(firstUserContent(out)).not.toContain("<thinking_mode>");
    expect(firstUserContent(out)).not.toContain("<max_thinking_length>");
  });
  ...
  it.each(["auto", "minimal", "ultra"])(
    ...
    (effort) => {
      ...
      expect(out.additionalModelRequestFields).toBeUndefined();
      expect(firstUserContent(out)).toContain("<thinking_mode>enabled</thinking_mode>");
      expect(firstUserContent(out)).toContain("<max_thinking_length>");
    }
  );
  ...
  it("sends Claude system in the first user content without a top-level systemPrompt", () => {
    const out = C2K({
  ...
  it("replays the frozen system-bearing first user turn while updating current time", () => {
    const credentials = {
      rawHeaders: { "x-session-id": "claude-kiro-system-replay" },
      connectionId: "kiro-system-replay-account",
    };
    ...
    try {
      ...
      expect(firstUserContent(first)).toBe(
        "stable instruction\n\n[Context: Current time is 2026-01-01T00:00:00.000Z]\n\nfirst"
      );
      expect(second.conversationState.history[0].userInputMessage.content).toBe(firstUserContent(first));
      expect(second.conversationState.currentMessage.userInputMessage.content).toBe(
        "[Context: Current time is 2026-01-01T00:01:00.000Z]\n\nsecond"
      );
    } finally {
      vi.useRealTimers();
    }
