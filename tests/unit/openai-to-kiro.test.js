...
const thinkingSurfaceOf = (result) => {
...
describe("openaiToKiroRequest", () => {
  ...
  describe("thinking budget", () => {
    it("maps reasoning_effort low to max_thinking_length 1024", () => {
      ...
      expect(thinkingSurfaceOf(result)).toContain("<max_thinking_length>1024</max_thinking_length>");
    ...
    it("maps reasoning_effort high to max_thinking_length 24576", () => {
      ...
      expect(thinkingSurfaceOf(result)).toContain("<max_thinking_length>24576</max_thinking_length>");
      expect(result.additionalModelRequestFields).toEqual({
        thinking: { type: "adaptive", display: "summarized" },
    ...
    it.each([
      ...
      expect(thinkingSurfaceOf(result)).not.toContain("<thinking_mode>");
      expect(thinkingSurfaceOf(result)).not.toContain("<max_thinking_length>");
    ...
    it.each(["auto", "minimal", "ultra"])(
      ...
      (effort) => {
        ...
        expect(result.additionalModelRequestFields).toBeUndefined();
        expect(thinkingSurfaceOf(result)).toContain("<thinking_mode>enabled</thinking_mode>");
        expect(thinkingSurfaceOf(result)).toContain("<max_thinking_length>");
      }
    );
    ...
    it.each([
      ...
      expect(result.additionalModelRequestFields).toBeUndefined();
      expect(thinkingSurfaceOf(result)).toContain("CHUNKED WRITE PROTOCOL");
    });
    ...
    it("clamps reasoning_effort max to Kiro max_thinking_length 32000", () => {
      ...
      expect(thinkingSurfaceOf(result)).toContain("<max_thinking_length>32000</max_thinking_length>");
    ...
    it("uses Claude thinking.budget_tokens as max_thinking_length", () => {
      ...
      expect(thinkingSurfaceOf(result)).toContain("<max_thinking_length>4096</max_thinking_length>");
    });
    ...
    it("uses the default budget for synthetic -thinking models with no explicit config", () => {
      ...
      expect(thinkingSurfaceOf(result)).toContain("<max_thinking_length>16000</max_thinking_length>");
    });

    it("keeps thinking prefix in user content and current time per turn", () => {
      ...
      expect(contentOf(first)).toContain("<max_thinking_length>");
      expect(contentOf(second)).toContain("<max_thinking_length>");
    ...
    it("does not inject thinking prefix for reasoning_effort none", () => {
      ...
      expect(thinkingSurfaceOf(result)).not.toContain("<thinking_mode>enabled</thinking_mode>");
      expect(thinkingSurfaceOf(result)).not.toContain("<max_thinking_length>");
      expect(result.additionalModelRequestFields).toBeUndefined();
    });
  });
